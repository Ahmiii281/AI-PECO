...
- `/backend` - Python FastAPI application serving general APIs and device commands
- `/ml` - Machine Learning pipeline (separate models, datasets, training, and inference)
- `/esp32` - Arduino code for ESP32 microcontroller with non-blocking reconnection
...
1. Ensure your MongoDB instance is running (or the app will auto-fallback to mock DB).
2. Run tests: `cd backend && pytest` (runs all auth, API, and ML tests).
